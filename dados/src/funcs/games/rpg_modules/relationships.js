...
async function iniciarNamoro(senderId, nomeNpc) {
    ...
    // TODO: Verificar outras condições (missão, tarefa)
    // if (cond.missao_requerida && !jogadorCompletouMissao(dadosJogador, cond.missao_requerida)) {
    //     return { msg: `❓ ${npc.nome} mencionou que você precisa completar a missão "${cond.missao_requerida}" antes de poderem se aproximar mais.` };
    // }
...
async function iniciarCasamento(senderId, nomeNpc) {
    ...
    // TODO: Adicionar bônus de casamento (se houver)
    // Ex: Bônus de XP, acesso a loja especial, etc.
...
async function terminarRelacionamento(senderId, nomeNpc) {
    ...
    if (statusAntigo === "casado" && dadosRelacionamento.conjuge === npcIdNormalizado) {
        ...
        // TODO: Lidar com filhos? (Remover? Ficam com o jogador?)
    }
